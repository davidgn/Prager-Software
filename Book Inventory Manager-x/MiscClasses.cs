...
namespace Prager_Book_Inventory
{
    ...
    //+++++++++++++++++++++++++++++++++++++++++++++++++++
    //--  populates the UserID and Passwords from DGV
    //+++++++++++++++++++++++++++++++++++++++++++++++++++
    class populateUIDs
    {
...
//            dgv1.Sort(dgv1.Columns[0], System.ComponentModel.ListSortDirection.Ascending);
        ...
        public void removeDuplicates(DataGridView dgv1, populateUIDs ui) {
            ...
            //  copy the first 24 rows to an array
            ArrayList rows = new ArrayList();
            ArrayList singleRow;
            for (int rowNdx = 0; rowNdx < 24; rowNdx++)  //  go through each of the first 24 rows
            {
                singleRow = new ArrayList();
                singleRow.Add(dgv1.Rows[rowNdx].Cells["ListingService"].Value);
                singleRow.Add(dgv1.Rows[rowNdx].Cells["UID"].Value);
                singleRow.Add(dgv1.Rows[rowNdx].Cells["PWD"].Value);
                singleRow.Add(dgv1.Rows[rowNdx].Cells["FTPAddr"].Value);
                singleRow.Add(dgv1.Rows[rowNdx].Cells["FTPDir"].Value);
                singleRow.Add(dgv1.Rows[rowNdx].Cells["FileFmt"].Value);
                rows.Add(singleRow);
            }
            ...
            //  insert the 24 rows back into the table    object o = ((ArrayList)rows[0])[0]; // When you want to retrieve the data...
            string insertString = "";
        ...
        public void saveDGVContents(DataGridView dgv1, mainForm mf) {
            ...
            foreach (DataGridViewRow dataRow in dgv1.Rows)  //  take each row and make an update statement with it
            {
                ...
                if (dataRow.Index < 19)  //  first 19 'named' sites
