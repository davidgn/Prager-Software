        ...
        private void uploadListings(string sFileName) {
            ...
            if (cbUploadBandN.Checked == true) {
                ...
                if (cbUploadTest.Checked)
                    ...
                    args[3] = BnNFileName.Substring(3, 17);  // use original filename
            ...
            * */
            ...
            //-->  Amazon.com (Tab)
            if (cbUploadAmazon.Checked == true || cbUploadAmazonUK.Checked == true) {
                //if (cbUploadAmazonUK.Checked && (AmazonUID.Length == 0 || AmazonPwd.Length == 0)) {
                //    MessageBox.Show("You must supply your email address and password", "Prager Book Inventory Manager", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                //    return;
                //}
                ...
                lbUploadStatus.Items.Insert(0, "upload to Amazon started (" + uploadFN1 + ")");
                lbUploadStatus.Refresh();

                //if (cbUploadAmazonUK.Checked) {  //   not used except for Az.co.UK
                //    args[0] = AmazonUID;
                //    args[1] = AmazonPwd;
                //}
            ...
            if (cbUploadChrislands.Checked == true) {
                ...
                if (cbUploadPurgeReplace.Checked == true) {
                    ...
                    if ((rc = uploadFile(args)) != 0)
                        lbUploadStatus.Items.Insert(0, "--> add/modify upload to Chrislands had an error: " + rc);
                    else {
                        args[3] = sFileName2.Replace(".txt", ".tab");
                        if ((rc = uploadFile(args)) != 0)
                            lbUploadStatus.Items.Insert(0, "--> delete upload to Chrislands had an error: " + rc);
                        else
