...
namespace Media_Inventory_Manager
{
    ...
    partial class mainForm : Form
    {
        ...
        private void xferDataToInvoice()
        {
            ...
            {
                MessageBox.Show("You must select a customer from the list above", "Prager Media Inventory Manager", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
        ...
        private void removeShoppingCartItem()
        {
            ...
            foreach (ListViewItem item in invdata)
            {
                ...
                //  now remove item from record in media table (item.text has SKU)
                string updateString = "UPDATE tMedia SET InvoiceNbr = null WHERE SKU = '" + item.Text + "'";
        ...
        public void copyShipAddressToClipboard(System.Windows.Forms.ListBox shipAddress)
        {
            ...
            {
                MessageBox.Show("Nothing to copy", "Prager Media Inventory Manager", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
        ...
        private void initializeShoppingCart()
        {
            if (lvShoppingCart.AllowColumnReorder == false)  //  first time flag...
            {
                ...
                // Attach Subitems to the ListView
                lvShoppingCart.Columns.Add("SKU", 110, HorizontalAlignment.Left);
                lvShoppingCart.Columns.Add("Title", 300, HorizontalAlignment.Left);
                lvShoppingCart.Columns.Add("UPC", 90, HorizontalAlignment.Left);
                lvShoppingCart.Columns.Add("Price", 61, HorizontalAlignment.Right);
            }

            if (shoppingCart.Count > 0)
            {
                string buildCommand = "select SKU, Title, UPC, Price from tMedia where  ";
                foreach (string dataString in shoppingCart)
                {
                    buildCommand += "SKU = '" + dataString.Trim() + "' or ";
                }
                ...
                while (dr.Read())
                {
                    if (lvShoppingCart.Items.Count > 0)  //  if listview is not empty
                    {
                        ...
                        {
                            if (lvShoppingCart.Items[x].SubItems[0].Text == (string)dr[0])  //  if item is already in the invoice, skip it...
                            {
                                skipIt = true;
                                break;
                            }
                    ...
                    if (!skipIt)
                    {
                        ...
                        lvi.SubItems.Add(dr[2].ToString());  //  UPC
        ...
        private void computeNewInvTotal()
        {
            ...
            decimal subAmount = decimal.Parse(tbOrderTotal.Text.ToString());
            CultureInfo ci = CultureInfo.CurrentCulture;
            NumberFormatInfo nfi = ci.NumberFormat;

            //  discount
            if (tbDiscount.Text.Length > 0 && tbDiscount.Text != ".")
                subAmount -= decimal.Parse(tbDiscount.Text.ToString());
            ...
            if (tbPayment.Text.Length > 0 && tbPayment.Text != ".")
                subAmount -= decimal.Parse(tbPayment.Text);
...
//-----------------------------------------------------------------------------------------
  ...
  //          XmlElement rootElem;
            ...
            for (int row = 0; row < itemCount; row++)
            {
                ...
                lineItem.SetAttribute("SKU", null, lvShoppingCart.Items[row].SubItems[0].Text);
                lineItem.SetAttribute("bookTitle", null, lvShoppingCart.Items[row].SubItems[1].Text);
                lineItem.SetAttribute("bookUPC", null, lvShoppingCart.Items[row].SubItems[2].Text);
