...
namespace Media_Inventory_Manager
{
    class AmazonUKUpload
    {
        ...
        public int uploadAmazonUKFiles(string[] args, ListBox lbUploadStatus) {
            ...
            const string UKaddModifyDeleteURL = @"https://secure.amazon.co.uk/exec/panama/seller-admin/catalog-upload/add-modify-delete";
            const string UKpurgeAndReplace = @"https://secure.amazon.co.uk/exec/panama/seller-admin/catalog-upload/purge-replace";
            ...
            uploadFilename1 = fileNameParts[0] + @"\" + fileNameParts[1] + @"\" + fileNameParts[2] + @"\" + fileNameParts[3].Replace("HB", "AZMu");
            uploadFilename2 = fileNameParts[0] + @"\" + fileNameParts[1] + @"\" + fileNameParts[2] + @"\" + fileNameParts[3].Replace("HB", "AZnoUPC");
            ...
            //  get length of input file with UPCs
            FileInfo fi = new FileInfo(uploadFilename1);
            if (!fi.Exists) { //  does file exist?
                System.Windows.Forms.MessageBox.Show("File " + uploadFilename1[0] + " is missing; did you change the export path?", "Prager Media Inventory Manager", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return -1;
            }
            ...
            ////  get length of input file with non-UPCs
            //fi = new FileInfo(uploadFilename2);
            //if (!fi.Exists) {  //  does file exist?
            //    System.Windows.Forms.MessageBox.Show("File " + uploadFilename2 + " is missing; did you change the export path?", "Prager Media Inventory Manager", MessageBoxButtons.OK, MessageBoxIcon.Error);
            //    return -1;
            //}
        ...
        private int uploadToAmazon(HttpWebRequest myRequest, ListBox lbUploadStatus) {
            ...
            try {
                ...
                System.Windows.Forms.MessageBox.Show("Error during upload to Amazon.com: " + ex.Message, "Prager Media Inventory Manager", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return -1;
            }
            return 0;
        }
