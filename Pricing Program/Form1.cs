...
namespace Prager_Pricing_Program
{
    ...
    partial class formISBNLookup : Form
    {
        ...
        static public string programVersion = "13.240";  //  good for 12 months from compileDate (BETA only good for 30 days)
        ...
        public static DateTime compileDate = DateTime.Parse("28 Aug 2013 11:24:00 AM", localCulture);
         ...
         *   
         ...
         -  fixed:  crash in pricingRoutines.c s (moved to this app)  (12.1.1)
         * */
...
9780596159764
        ...
        private void bSearch_Click(object sender, EventArgs e) {
            ...
            foreach (string line in lines) {  //  for each ISBN in the list...
                if (line.Length == 0)
                    continue;
                ...
                //  instantiate
                BookInfo.PricingRoutines bpr = new PricingRoutines();
                BookInfo.PricingRoutines.BookData bD = new PricingRoutines.BookData();
                BookInfo.PricingRoutines.PricingData pD = new PricingRoutines.PricingData();

                if (!bpr.getInternetPrices(line.Trim(), bD, pD)) {  //  found nothing!
                ...
                int lbCount = bD.bookList.Keys.Count;
                ...
                foreach (string key in bD.bookList.Keys) {
                    ...
                    if (recordCount == 0) {  //  first two lines
                        ListViewItem lvi = new ListViewItem("ISBN: " + bD.ISBN);  //  add ISBN
                        lvi.BackColor = Color.LightSteelBlue;
                        lvArray[listViewPointer].Items.Add(lvi);  // Add the list items to the ListView

                        ListViewItem lvi0 = new ListViewItem("List Price: " + bD.listPrice);  //  add list price
                        lvi0.BackColor = Color.LightYellow;
                        lvArray[listViewPointer].Items.Add(lvi0);  // Add the list items to the ListView
                    }

                    string bookCondition = "";
                    switch (bD.bookList[key].bookCondn.ToString()) {
                        case "g":
                            bookCondition = "Good";
                            break;
                        case "u":
                    ...
                    ListViewItem lvi1 = new ListViewItem(bD.bookList[key].venueName);  //  add venue name
                    lvi1.SubItems.Add(bookCondition);  //  add condition
                    lvi1.SubItems.Add(bD.bookList[key].price);  //  and price

                    accumulatedPrice += decimal.Parse(bD.bookList[key].price);
                    recordCount++;

                    if (bD.bookList[key].venueName.Contains("Invalid 10 digit ISBN"))
                ...
                if (lbCount > 1)
                    avgPrice = accumulatedPrice / (lbCount - 1);
