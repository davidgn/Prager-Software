...
//---------------------------------------------------------------------------------------
        private int parseAddallPricingInfo(string webPageString, Label lp, ListView lv, string sisbn)
        {
            ...
            int indx = 1;  //  indicate next line number to start on (prior to ++indx)
            ...
            int divisor = lv.Items.Count - 2;
            decimal holdingAmount = accumulatedPrice / divisor;
            lv.Items.Add(" ");  //  space
            lv.Items.Add("Average price:");
            string avgPrice = Convert.ToString(decimal.Round(holdingAmount, 2));
            lv.Items[indx+2].SubItems.Add(avgPrice.Trim());
            lv.Refresh();
...
//            lv.Items[indx].SubItems.Add(System.Convert.ToDecimal(price).ToString());
            lv.Items[indx].SubItems.Add(price.Trim());
            lv.Refresh();
...
//        catch (ArgumentOutOfRangeException)
  //  {
    //}
            ...
            lv.Items[lv.Items.Count-1].SubItems.Add(avgPrice.Trim());
            lv.Refresh();
