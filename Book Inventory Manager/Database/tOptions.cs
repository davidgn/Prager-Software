...
namespace Prager_Book_Inventory
{
    ...
    partial class mainForm : Form
    {
        ...
        internal static string AmazoncaUID = "";
        internal static string AmazoncaPwd = "";
        internal static string AmazoncoUKUID = "";
        internal static string AmazoncoUKPwd = "";
        ...
        public void restoreOptions(string optionsXMLFilename) {
            ...
            if (File.Exists(xmlFilename)) {
                using (XmlReader tr = XmlReader.Create(xmlFilename)) {
                    try {
                        while (tr.Read()) {  // If the node has value 
                            switch (tr.Name) {
                                ...
                                case "ProgramOptions":
                                    ...
                                    if (tr.GetAttribute("oDontSortCatalog") == "true")
                                        cbDontSortCatalog.Checked = true;
                                    else
                                        cbDontSortCatalog.Checked = false;
                                    ...
                                    if (tr.GetAttribute("oAmazonUK") == "true")
                                        rbAmazonUK.Checked = true;
                                    else
                                        rbAmazonUK.Checked = false;

                                    if (tr.GetAttribute("ocbUseAmazon4BookInfo") == "true")  //  added 12.8.6
                                        cbUseAmazon4BookInfo.Checked = true;
                                    else
                                        cbUseAmazon4BookInfo.Checked = false;

                                    if (!rbAmazonUS.Checked && !rbAmazonCA.Checked && !rbAmazonUK.Checked)  //  first time
                                        rbAmazonUS.Checked = true;
                        ...
                        //  Amazon keys
                        FbCommand cmd1 = new FbCommand();  //  see where the data is...
                        string selectStr = "SELECT COUNT(*) FROM tAZUID WHERE MWSMERCHANTID != ''";
                        cmd1.CommandText = selectStr;
                        cmd1.Connection = bookConn;
                        int fullCount = (int)cmd1.ExecuteScalar();

                        if (fullCount == 0) {  //  if not in d/b, then copy from Options.xml file
                    ...
                    if (cbEnableCR.Checked)
                        fTrace("I - Crash Reporting enabled");
                    else
                        fTrace("I - Crash Reporting NOT enabled");
                }
            }
            else {
                if (DateTime.Compare(installedDate.Date, DateTime.Today.Date) != 0)  //  if not installed today
            ...
            if (dr[0].ToString().Length != 0) {  //  UK doesn't use these keys yet...
                tbMerchantID.Text = dr[0].ToString();
                tbMarketplaceID.Text = dr[1].ToString();
            }
            if (dr[2].ToString().Length != 0) {  //  UK only uses these keys...
        ...
        public void saveOptions(string optionsXMLFilename) {
            ...
            * */
            ...
            tw.WriteAttributeString("oAmazonUK", rbAmazonUK.Checked ? "true" : "false");
            tw.WriteAttributeString("oHalfDotComToken", tbHalfToken.Text);
            tw.WriteAttributeString("oDontSortCatalog", cbDontSortCatalog.Checked ? "true" : "false");
            ...
            tw.WriteAttributeString("ocbUseAmazon4BookInfo", cbUseAmazon4BookInfo.Checked ? "true" : "false");
            tw.WriteEndElement();
            ...
            tw.WriteAttributeString("uAmazon", cbUploadAmazon.Checked == true && tbMerchantID.Text.Length > 0 ? "true" : "false");
            tw.WriteAttributeString("uAmazonUK", cbUploadAmazonUK.Checked == true && tbMerchantID.Text.Length > 0 ? "true" : "false");
...
*/
            ...
            ////  update tUploadInfo table
            //populateUIDs pUID = new populateUIDs();
            //pUID.cleanDGVtable(UIDdataGridView);
            ...
            " AWSSecretKey = '" + tbAWSSecretKey.Text + "' ROWS 1";
        ...
        public int checkInstallationDate() {
            ...
            string updateString = "UPDATE tOptions SET tInstallDate = CURRENT_DATE, MAC = '" + MACAddress + "' ROWS 1";
            ...
            try { //  initial open of Firebird database
                ...
                fTrace("E - 1139: " + ex.Message.Substring(0, 75));
                return -1;
            }
            ...
            if (!rdr.Read() || rdr.IsDBNull(0)) {  //  if installDate is missing, then this is the first time...
                ...
                createUploadInfoTable();  //  first time, fill UID and Password table
            }
            else
        ...
        public void restoreMapping() {
            ...
            if (rbImportBT.Checked) {
                tbMapBookNbr.Text = "Book_ID";
                tbMapAuthor.Text = "Author";
                tbMapTitle.Text = "Title";
                tbMapAddTitle.Text = "Subtitle";
                tbMapIllus.Text = "Illustrator";
                tbMapPublisher.Text = "Publisher";
                tbMapDesc.Text = "Description";
                tbMapAddDesc2.Text = "Defects";
                tbMapAddDesc3.Text = "Notes";
                tbMapPrice.Text = "Retail_Price";
                tbMapBinding.Text = "Binding";
                tbMapEdition.Text = "Edition";
                tbMapCost.Text = "Cost";
                tbMapYearPub.Text = "Date";
                tbMapBookSize.Text = "Size";
                tbMapDateSold.Text = "DateSold";
                tbMapISBN.Text = "ISBN";
                tbMapDJCond.Text = "DJ";
                tbMapBookCond.Text = "Condition";
                tbMapPubLoc.Text = "Place";
                tbExpedited.Text = "Expedite";
                tbInternational.Text = "International";
                tbMapNbrPages.Text = "Pages";
                tbMapPrivateNotes.Text = "Notes";
                tbMapLocation.Text = "Location";
                tbNbrOfCopies.Text = "Quantity";
                tbMapWeight.Text = "Weight";
                tbMapStatus.Text = "Status";
            }
            else {
                ...
                if (File.Exists(xmlFilename)) {
                    ...
                    while (tr.Read())   {  // If the node has value
                        ...
                        tbMapAddTitle.Text = tr.GetAttribute("MapTitle");
        ...
        //+++++++++++++++++++++++++++++++++++++++++++++++
        //-- user has changed a User ID or password; update variables
        //+++++++++++++++++++++++++++++++++++++++++++++++
