...
namespace BookInfo
{
    ...
    public class PricingRoutines
    {
     ...
     //       Thread.CurrentThread.CurrentCulture = new CultureInfo("en-GB");  //  TESTING ONLY!

            if (CultureInfo.CurrentCulture.Name == "en-US") {
                page = readFindBookPricesDotCom(isbn);
                rc = parseFindBookPricesDotCom(page, isbn, bD, pD);
            }
            else if (CultureInfo.CurrentCulture.Name == "en-GB") {
                page = readABEBooksDotCoDotUK(isbn);
                rc = parseABEBooksDotCoDotUK(page, isbn, bD, pD);
        ...
        private string readFindBookPricesDotCom(string isbn) {
        ...
        private bool parseFindBookPricesDotCom(string page, string isbn, BookData bD, PricingData pD) {
        ...
        //-------------------------------------------------------------------
        //--    read ABEBooks.co.uk
        private string readABEBooksDotCoDotUK(string isbn) {

            Cursor.Current = Cursors.AppStarting;

            System.Net.ServicePointManager.MaxServicePointIdleTime = 10000;

            Uri uri = new Uri(@"http://www.abebooks.co.uk/servlet/SearchResults?isbn=" +
                isbn + @"&y=12&x=105");
            HttpWebRequest request = (HttpWebRequest)WebRequest.Create(uri);
            request.Referer = "http://www.abebooks.co.uk";
            request.KeepAlive = false;
            request.Method = "POST";
            request.ContentType = "application/x-www-form-urlencoded";
            request.ContentLength = 0;
        ...
        //--------------------------------------------------------------
        //--  parse ABEBooks.co.uk
        private bool parseABEBooksDotCoDotUK(string page, string isbn, BookData bD, PricingData pD) {
            ...
            if (page.Contains("The ISBN provided is not valid"))
                return false;
            ...
            //--  do selling prices 
            r = new Regex("<div class=\"author\">");  //  find starting point of selling prices (m.Index)
            m = r.Match(page, 0);
            if (m.Success) {
                startIndex = m.Index;  //  position it...
                ...
                do {  //  now, go through each store and price 
                    ...
                    //--  find store name
                    r1 = new Regex("<div class=\"bookseller\">");
                    m1 = r1.Match(page, startIndex);
                    if (m1.Success) {  //  found it..
                        r = new Regex("title=\"");
                        m = r.Match(page, m1.Index + 120, 100);

                        if (m.Success) {  //  store name was found...
                            r1 = new Regex("\">");  //  now find end of name
                            m1 = r1.Match(page, m.Index + 7, 50);
                            if (m1.Success) {
                                pD = new PricingData();
                                pD.venueName = page.Substring(m.Index + 7, m1.Index - (m.Index + 7));  //  move venue name
                            }

                            //--  find price 
                            r = new Regex("<div class=\"item-price\">");
                            m = r.Match(page, m1.Index + 1000, 500);  //  m -> beginning of price string

                            if (m.Success) {  //  m -> beginning of price
                                r1 = new Regex(@"</span></div>");  //  find end of price string
                                m1 = r1.Match(page, m.Index + 58, 20);

                                if (m1.Success) //  m1 -> end of price
                                    pricingData = page.Substring(m.Index + 60, m1.Index - (m.Index + 60));
                            ...
                            //--  find beginning of book condition
                            r = new Regex("Book Condition: ");
                            m = r.Match(page, m1.Index + 800, 400);

                            if (m.Success) {  //  find end of condition
                                r1 = new Regex(". ");
                                m1 = r1.Match(page, m.Index + 16, 25);  //  m -> end of condition

                                if (m1.Success) {
                                    conditionData = page.Substring(m.Index + 16, m1.Index - (m.Index + 16));
                                    switch (conditionData.ToLower()) {
                                        case "good":
                                            pD.bookCondn = 'g';
                                            break;
                                        case "fair":
