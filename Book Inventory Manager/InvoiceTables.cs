...
namespace Prager_Book_Inventory
{
    partial class mainForm : Form
    {
        ...
        public void fillCustomerListView()
        {
            ...
            FbDataAdapter da = new FbDataAdapter(commandString, bookConn);
        ...
        private void addCustomerInfo()
        {
            ...
            if (cbAutoCustomerNbr.Checked && tbCustID.Text.Length == 0)
            {
                ...
                else  //  creating first customer number on file
                {
                    ...
                    {
                        MessageBox.Show("Enter starting Customer number in text box", "Prager Book Inventory Manager", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        return;
                    }
            ...
            else  //  autogenerate == false OR tbInvoiceNbr.Text.Length > 0
            {
                ...
                if (tbCustID.Text.Length == 0)
                {
                    MessageBox.Show("Customer Number (key) missing", "Prager Book Inventory Manager", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
            }
            ...
            if (cbSameAsBillingInfo.Checked)
            {
        ...
        private void PopulateInvoiceDetailPanel(string invoiceNbr)
        {
            ...
            string strSQL = "SELECT i.*, b.* from tInvoice i LEFT JOIN tBooks b ON i.tInvoiceNbr = b.InvoiceNbr WHERE i.tInvoiceNbr = '" + invoiceNbr + "'";
            DataSet ds = new DataSet();

            if (bookConn.State == ConnectionState.Open)  //  it is the da's responsibility to open the connection
                bookConn.Close();

            FbDataAdapter da = new FbDataAdapter(strSQL, bookConn);
            ...
            //  search for books that are in this order and populate shopping cart
            for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
            {
                if (ds.Tables[0].Rows[i].RowState != DataRowState.Deleted)  // Only row that have not been deleted
                {
                    ListViewItem lvi = new ListViewItem(ds.Tables[0].Rows[i]["BookNbr"].ToString());
                    lvi.SubItems.Add(ds.Tables[0].Rows[i]["Title"].ToString());
                    lvi.SubItems.Add(ds.Tables[0].Rows[i]["ISBN"].ToString());
        ...
        private void updateCustomerTable()
        {
            ...
            {
                MessageBox.Show("You can not update a record before adding it", "Prager Book Inventory Manager", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
        ...
        private void deleteRecordFromCustomerTable()
        {
            ...
            FbCommand myCommand = new FbCommand(commandString, bookConn);
        ...
        {
            //  are there any books in invoice?

            if (lvShoppingCart.Items.Count == 0)
            {
                MessageBox.Show("There are no books in shopping cart!", "Prager Book Inventory Manager", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            ...
            if (cbAutoInvoiceNbr.Checked == true && tbInvoiceNbr.Text.Length == 0)
            {
                ...
                else  //  creating first invoice on file
                {
                    ...
                    {
                        MessageBox.Show("Enter numeric starting invoice number in text box", "Prager Book Inventory Manager", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        return;
                    }
            ...
            else  //  autogenerate == false OR tbInvoiceNbr.Text.Length > 0
            {
                if (tbInvoiceNbr.Text.Length == 0)
                {
                    MessageBox.Show("Enter Invoice number in text box and/or check AutoGenerate in Options menu", "Prager Book Inventory Manager", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
            ...
            {
                MessageBox.Show("Error:  you must choose a customer from the customer tab and Transfer Data to Invoice", "Prager Book Inventory Manager", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            ...
            catch (Exception ex)
            {
                ...
                {
                    MessageBox.Show("Invoice number already exists in database; change or clear Invoice number and try again", "Prager Book Inventory Manager", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
                else
                {
                    MessageBox.Show("537 - error adding Invoice: " + ex.Message, "Prager Book Inventory Manager", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
            }

            updateBookWithInvoiceNbr(true);   //  now, update tBooks with invoice number for each book in list 
        ...
        private void updateInvoiceTable()
        {
            ...
            if (tbInvoiceNbr.Text.Length == 0)
            {
                MessageBox.Show("Error:  you can not do an Update without establishing an existing Invoice number", "Prager Book Inventory Manager", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            ...
            try {
                ...
                lUpdateStatus.Text = "Update NOT Successful";
        ...
        private void deleteRecordFromInvoiceTable()
        {
            ...
            updateBookWithInvoiceNbr(false);  //  delete invoice number from book(s) in invoice table
        ...
        //------------------------------------------------------------------------    update book in table with invoice number    -------------------------------------------
        private void updateBookWithInvoiceNbr(bool updateFlag)  //  true = update, false = delete
        {
            //  get book numbers
            ...
            {
                bookNumber = lvShoppingCart.Items[i].SubItems[0].Text;
                if (bookNumber.Length > 0)
                {
                    if (updateFlag == true)
                        updateString = "UPDATE tBooks SET InvoiceNbr = '" + tbInvoiceNbr.Text + "' WHERE BookNbr = '" + bookNumber + "'";
                    else  //  delete book number
                        updateString = "UPDATE tBooks SET InvoiceNbr = null WHERE BookNbr = '" + bookNumber + "'";
                    FbCommand cmd = new FbCommand(updateString, bookConn);
                    cmd.ExecuteNonQuery();
                }

                updateDataBasePanel(bookNumber);  // now, repaint the database listview
            }
        ...
        private void searchByInvoiceNbr()
        {
            ...
            if (tbInvoiceNbr.Text.Length == 0)
            {
                MessageBox.Show("Search argument missing", "Prager Book Inventory Manager", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
